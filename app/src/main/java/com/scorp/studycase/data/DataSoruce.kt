package com.scorp.studycase.data
...
data class FetchError(val errorDescription: String, val errorCode: Int)
...
class DataSource @Inject constructor() {
    ...
    private fun processRequest(next: String?): ProcessResult {
        ...
        if (isError) {
            ...
            error =
                FetchError(errorDescription = "Internal Server Error", errors.INTERNAL_SERVER_ERROR)
        } else {
            ...
            if (next != null && (nextIntValue == null || nextIntValue < 0)) {
                error = FetchError(errorDescription = "Parameter error", errors.PARAMETER_ERROR)
            } else {
